...
//    NSLog(@"%@--%@ --%d---%lu",model.borrowTime,model.isbn,[model.borrowTime containsString:@"null"],(unsigned long)model.isbn.length);
    if (model.isbn.length > 0) {
        self.bookInfoLabel.text = [NSString stringWithFormat:@"%@\n%@\n%@",model.publisher,model.bookId,model.isbn];
    }else {
